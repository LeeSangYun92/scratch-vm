...
class Scratch3ModiBlocks {
    ...
    getInfo () {
        return {
            ...
            blocks: [
                ...
                {
                    opcode : 'speakerOn',
                    text : 'SPEAKER[NUM]번 크기[VOLUME]로 켜기',
                    blockType : BlockType.COMMAND,
                    arguments : {
                        NUM : {
                            type: ArgumentType.NUMBER,
                ...
                {
                    opcode : 'displayText',
                    text : 'DISPLAY[NUM]번 화면 메시지 [MSG]로 띄우기',
                    blockType : BlockType.COMMAND,
                    arguments : {
                        NUM : {
                            type: ArgumentType.NUMBER,
                ...
                {
                    opcode : 'motorSpeed',
                    text : 'MOTOR[NUM]번 상단은 속도[UP_SPEED], 하단은 속도[DOWN_SPEED] 회전하기',
                    blockType : BlockType.COMMAND,
                    arguments : {
                        NUM : {
                            type: ArgumentType.NUMBER,
    ...
    ledRGB(args){
        let num = Cast.toNumber(args.NUM);
        let r = Cast.toNumber(args.R);
        let g = Cast.toNumber(args.G);
        let b = Cast.toNumber(args.B);
        
        req.ledRGB(num,r,g,b);  
    }
    ledOn(args){
        let num = Cast.toNumber(args.NUM);
        req.ledOn(num);
    ...
    {
        let num = Cast.toNumber(args.NUM);
        let msg = Cast.toString(args.MSG);
        req.displayText(num,msg);
    }
    motorSpeed(args)
    {
        let num = Cast.toNumber(args.NUM);
        let up_speed = Cast.toNumber(args.UP_SPEED);
        let down_speed = Cast.toNumber(args.DOWN_SPEED);
        req.motorSpeed(num,up_speed,down_speed);
    }
    buttonPressed()
    {   
        // var res =request('POST','http://localhost:3000/button/pressed');
